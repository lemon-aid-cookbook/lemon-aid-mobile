...
import {GetDetailPostNotNav, UpdateRecipe} from 'pages/Profile/redux/actions';
...
const UpdatePostPage: React.FC<Props> = (props) => {
  const {goBack, navigate} = useNavigation();
  const postId = useNavigationParam('id');
  const detailPost = useSelector((state) => state.Profile.detailPost);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(GetDetailPostNotNav.get({postId}));
  }, []);
      ...
      <ScrollView style={styles.listWrap} showsVerticalScrollIndicator={false}>
        <Formik
          initialValues={{
            ...
            ration: Number(detailPost.ration),
            cookingTime: detailPost.cookingTime,
            difficultLevel: detailPost.difficultLevel,
            ingredients: detailPost.ingredients,
            categories:
              detailPost.categories && detailPost.categories[0] !== ''
                ? detailPost.categories
                : [],
            hashtags: detailPost.hashtags,
            steps: detailPost.content,
          }}
...
UpdatePostPage.defaultProps = defaultProps;
